...
// Map to store category IDs for reference by other seed scripts
export const categoryIdMap: Map<string, string> = new Map();
export const subCategoryIdMap: Map<string, string> = new Map();

/**
 * Seeds categories and sub-categories for a user
 * @param userId - The user's UUID
 * @returns Object with category and subcategory ID maps
 */
export async function seedCategories(userId: string): Promise<{
  ...
  for (let i = 0; i < SEED_CATEGORIES.length; i++) {
    ...
    if (category.subCategories && category.subCategories.length > 0) {
      for (let subIndex = 0; subIndex < category.subCategories.length; subIndex++) {
        ...
        subCategoryIdMap.set(sub.id, insertedSub.id);
        subCategoryCount++;
      }
    }
  }
  ...
  console.log(`   ✓ Created ${subCategoryCount} sub-categories`);

  return {
    categoryIds: categoryIdMap,
    subCategoryIds: subCategoryIdMap,
  };
}
